...
def generate_header_contents(f_len, password, ver, key_amount, pbkdf2_iterations):
	...
	#print(header)
	final_key_split = []
	#Generating 512-bit (in hex) salt blocks. 
	#Then using PBKDF2 to encrypt a 512-bit key with the salt and password.
	#The encrypted key is concatenated.
...
def encrypt_file(filename,passtouse,ver,key_par, iter_k, mode="N", bytein=b"", hdrmode="PSK"):
	...
	f_hash = hmac.new(hmackey_enc,msg=enc_file,digestmod=hashlib.sha512).digest()
	print("HMAC:",bytes.decode(binascii.hexlify(f_hash)))
	enc_file.extend(f_hash)
	if mode == "N":
...
def decrypt_file(filename,passtouse, test_decrypt, mode="N", hdrmode="PSK", hdrcontents=""):
	...
	length = int(hlen,16)
	d_file = bytearray()
	timestodecrypt = (len(efile_r) // 64)-1
	...
	#print("HMAC key:",bytes.decode(binascii.hexlify(hmackey_enc)))
	fcalc_hash = hmac.new(hmackey_enc,msg=efile_r[:-64],digestmod=hashlib.sha512).digest()
	print("HMAC CALC:",bytes.decode(binascii.hexlify(fcalc_hash)))
	print("HMAC READ:",bytes.decode(binascii.hexlify(efile_r[-64:])))
	autoremove = True
	if test_decrypt == True:
		if fcalc_hash == efile_r[-64:]:
